...
$$\begin{aligned}
    P(S|X\setminus S)&= \frac{P(X)}{P(X\setminus S)} \\
    &= \frac{\prod_{X}P(X_i|Pa_i)}{\prod_{X\setminus S}P(X_i|Pa_i)} \\
...
where $X$ is the set of all the variable in the directed acyclic graph.
...
Where $U$ is a subset of set $X$,and $U=\lbrace S,B,T\rbrace$.
